...
const ProfileScrean = ({location, history}) => {
   ...
   const {name} = userDetails.user.userLogin.userIngo.name 
   console.log(name)
    ...
    useEffect(() => {
        ...
        setName(userDetaill&&userDetaill.name)
        setEmail(userDetaill.email)
    }, [dispatch, history, userInfo, userDetaill])

    const submitHandler = (e) => {
        e.preventDefault()
        dispatch(updateUserProfile({id: userDetaill._id, name, email, password }))
